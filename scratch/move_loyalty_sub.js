...
if (configStart !== -1 && loyaltyStart !== -1 && subStart !== -1) {
    // Loyalty inner bounds
    const loyaltyInnerStart = loyaltyStart + 1; // <div className="space-y-6 ...">
    let loyaltyEnd = loyaltyStart + 1;
    let nesting = 0;
    while (loyaltyEnd < lines.length) {
        if (lines[loyaltyEnd].includes("{view ===")) break; // safety
        if (lines[loyaltyEnd].includes("<div")) nesting += (lines[loyaltyEnd].match(/<div/g) || []).length;
        if (lines[loyaltyEnd].includes("</div")) nesting -= (lines[loyaltyEnd].match(/<\/div/g) || []).length;
        if (nesting === 0 && lines[loyaltyEnd].includes(")}")) {
            break;
        }
        loyaltyEnd++;
    }
    const loyaltyContentLines = lines.slice(loyaltyInnerStart, loyaltyEnd - 1); // exclude )}, include closing </div>
    ...
    while (subEnd < lines.length) {
        ...
        if (lines[subEnd].includes("<div")) nesting += (lines[subEnd].match(/<div/g) || []).length;
        if (lines[subEnd].includes("</div")) nesting -= (lines[subEnd].match(/<\/div/g) || []).length;
        if (nesting === 0 && lines[subEnd].includes(")}")) {
            break;
        }
        subEnd++;
    }
    ...
    // Delete sub block first to not mess up earlier indices
    lines.splice(subStart, subEnd - subStart + 1);
    
    // Delete loyalty block
    lines.splice(loyaltyStart, loyaltyEnd - loyaltyStart + 1);

    // Find insertion point inside config view
    let configEndIdx = loyaltyStart - 1;
    while(configEndIdx > configStart && !lines[configEndIdx].includes(")}")) {
        configEndIdx--;
    }
    
    // configEndIdx is `)}`. configEndIdx - 1 is `</div>` of space-y-6. configEndIdx - 2 is `</div>` of space-y-4.
    // So we insert at configEndIdx - 1
